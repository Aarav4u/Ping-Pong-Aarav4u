...
// Default AI level (will be set after prompt)
let aiLevel = 0.1;
...
function update() {
    ...
    // Determine which paddle is being hit by the ball and handle
...
function render() {
    ...
    drawRect(0, 0, canvas.width, canvas.height, "SKYBLUE");
    drawNet();
    ...
    // Draw paddles
    drawRect(user.x, user.y, user.width, user.height, "#88ff00ff");
    drawRect(com.x, com.y, com.width, com.height, "#002fffff");
...
// Start the game loop immediately so the game is visible
const framePerSec = 60;
setInterval(gameLoop, 1000 / framePerSec);

// Prompt for difficulty after game is visible
setTimeout(() => {
    let difficulty = prompt("Select difficulty: Easy, Medium, or Hard", "Medium");
    if (difficulty) {
        difficulty = difficulty.trim().toLowerCase();
        if (difficulty === "easy") aiLevel = 0.04;
        else if (difficulty === "hard") aiLevel = 0.18;
        else aiLevel = 0.1; // Medium or any other input
    }
}, 100);